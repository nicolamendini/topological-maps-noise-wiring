...
def collect_and_plot_grating_umap_3d(
    ...
    for _ in tqdm(range(n)):
        ...
        img = generate_sinusoidal_grating(crop_size, wavelength, angle, phase).unsqueeze(0).to(device)
    ...
    clean_embedding = clean_reducer.fit_transform(clean_codes_np)
    layer_name = "L3" if use_l3 else "L4"
    clean_title = f"{layer_name} clean codes, UMAP fit on clean"
    ...
    if include_noise:
        ...
        if fit_umap_on_noise:
            ...
            noisy_embedding = noisy_reducer.fit_transform(noisy_codes_np)
            noisy_title = f"{layer_name} noisy codes, UMAP fit on noisy"
        else:
            noisy_reducer = clean_reducer
            noisy_embedding = clean_reducer.transform(noisy_codes_np)
            noisy_title = f"{layer_name} noisy codes, transformed onto clean-fit UMAP"
...
def fit_and_plot(
    ...
    if y.dim() == 1:
        ...
        if rep_spread == "sem":
            ...
            raise ValueError("rep_spread must be 'sem', 'std', or 'none'.")
    else:
